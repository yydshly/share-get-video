...
REAL_NO_TITLE_SINGLE_NEWLINE_REPORT = """今日AI研究呈现多维突破：多语言推理方面，AdaMame解决大型推理模型的语言崩溃问题；智能体安全领域，OSGuard基准填补计算机操作安全评估空白，CoRA框架增强推理可靠性；效率优化上，NVIDIA开源5500亿参数混合专家模型，字节跳动发布万亿参数Mamba-Transformer融合架构；此外，低资源语言评估与数学形式化证明等领域也有重要进展。

AdaMame提出两阶段训练方案，在12种语言上实现准确率与语言一致性的帕累托最优平衡。
依据： 依据 1
OSGuard基准以动作级和端到端双粒度评估AI代理安全，防止"成功但有害"的捷径行为。
依据： 依据 1
NVIDIA Nemotron 3 Ultra开源5500亿参数MoE模型，推理吞吐量提升约6倍，精度相当。
依据： 依据 1
CoRA框架通过GRPO优化置信度与推理对齐，降低误差最多26.51%，提升模型校准能力。
依据： 依据 1
字节跳动Ling/Ring 2.6万亿参数模型引入混合线性注意力，兼顾即时响应与深层推理。
依据： 依据 1
研究表明vanilla基线在同等token预算下匹配或超越技能和记忆增强方法。
依据： 依据 1
低资源语言测试显示现有模型缺乏孔卡尼语能力，揭示多语言NLP区域文化理解不足。
依据： 依据 1"""
...
def test_report_profile_no_title_single_newline_item_evidence_pairs():
    result = generate_information_structure(
        REAL_NO_TITLE_SINGLE_NEWLINE_REPORT,
        compression_mode="strict",
        target_point_count="all",
        input_profile="report_overview_items",
    )
    ...
    assert result["stats"]["detectedItemCount"] == 7
    assert len(result["items"]) == 7
    for item, fragment in zip(result["items"], expected_fragments):
        assert fragment in item["description"]
        assert item["title"]
        assert item["titleSource"] in ("auto_generated", "fallback")
        assert item["evidence"] == ["依据 1"]

    serialized = serialize_for_visual_compose(result)
    assert serialized.count("【信息点") == 7
    item_area = serialized.split("【尾部总结】", 1)[0]
    assert item_area.count("标题：") >= 7
    assert item_area.count("描述：") == 7
...
def test_report_profile_conclusion_does_not_become_item():
    ...
    assert "整体来看" in result["conclusion"]["text"]


def test_report_profile_requested_conclusion_falls_back_to_final_source_clause():
    result = generate_information_structure(
        REAL_NO_TITLE_SINGLE_NEWLINE_REPORT,
        compression_mode="strict",
        target_point_count="all",
    ...
    assert result["conclusion"]["text"]
    assert "低资源语言评估" in result["conclusion"]["text"]
...
class TestSummaryOnlyReportItems:
    SUMMARY_ONLY_REPORT = """今日 AI 前沿速览：多个方向出现更新。

总结：OpenAI 发布新模型，推理能力提升，但成本仍需观察。

总结：Claude Code 新增能力，开发者可以更方便地完成代码修改和测试。"""

    def test_summary_label_is_body_not_literal_title(self):
        result = generate_information_structure(
            self.SUMMARY_ONLY_REPORT,
            compression_mode="strict",
            target_point_count="all",
            input_profile="report_overview_items",
        )
        ...
        assert result["stats"]["detectedItemCount"] == 2
        assert all(item["title"] != "总结" for item in result["items"])
        assert all(item["description"] for item in result["items"])
        assert all(item["summary"] == item["description"] for item in result["items"])

    def test_summary_only_items_serialize_with_title_and_body(self):
        result = generate_information_structure(
            self.SUMMARY_ONLY_REPORT,
            compression_mode="strict",
            target_point_count="all",
        ...
        assert serialized.count("【信息点") == 2
        assert serialized.count("描述：") == 2
        item_area = serialized.split("【尾部总结】", 1)[0]
        assert "标题：总结" not in item_area
